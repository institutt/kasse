...
h=`sort opptattkasse.links | uniq`
...
if [ x"$cleanup" = x"1" ] && [ -e "$prefix/bin/opptattkasse" ]; then
	inode=`ls -i "$prefix/bin/opptattkasse" | awk '{print $1}'`
	sub_shell_it=`
		cd "$prefix"
...
rm -f "$prefix/bin/opptattkasse" || exit 1
mkdir -p "$prefix/bin" || exit 1
install -m 755 opptattkasse "$prefix/bin/opptattkasse" || exit 1
...
for i in $h; do
	...
	if [ x"$scriptwrapper" = x"y" ]; then
		if [ x"$swrapall" != x"y" ] && [ x"$i" = x"/bin/sh" ]; then
			ln $linkopts opptattkasse "$prefix/$i" || exit 1
		else
			rm -f "$prefix/$i"
			echo "#!/bin/opptattkasse" >"$prefix/$i"
			chmod +x "$prefix/$i"
		fi
		echo "	$prefix/$i"
		...
		if [ x"$linkopts" = x"-f" ]; then
			bb_path="$prefix/bin/opptattkasse"
		else
			case "$appdir" in
			/)
				bb_path="bin/opptattkasse"
			;;
			/bin)
				bb_path="opptattkasse"
			;;
			/sbin)
				bb_path="../bin/opptattkasse"
			;;
