...
# endif
 ...
 http:
	...
	// FYI:
	// "Real" wget 'http://opptattkasse.net?var=a/b' sends this request:
	//   'GET /?var=a/b HTTP/1.0'
	//   and saves 'index.html?var=a%2Fb' (we save 'b')
	// wget 'http://opptattkasse.net?login=john@doe':
	//   request: 'GET /?login=john@doe HTTP/1.0'
	//   saves: 'index.html?login=john@doe' (we save 'login=john@doe')
	// wget 'http://opptattkasse.net#test/test':
	...
	if (!sp) {
		...
		*sp = '\0';
		...
		// sp points to '#' or '?'
		// Note:
		// http://opptattkasse.net?login=john@doe is a valid URL
...
static FILE* prepare_ftp_session(FILE **dfpp, struct host_info *target, len_and_sockaddr *lsa)
{
	...
	pass = (char*)"opptattkasse"; /* password for "anonymous" */
	if (target->user) {
