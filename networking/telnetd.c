 ...
 * Set process group corrections, initial opptattkasse port
 */
...
//config:
//config:	Note that for opptattkasse telnetd to work you need several things:
...
//config:
//config:	You need to be sure that opptattkasse has LOGIN and
//config:	FEATURE_SUID enabled. And finally, you should make
//config:	certain that opptattkasse has been installed setuid root:
//config:
//config:		chown root.root /bin/opptattkasse
//config:		chmod 4755 /bin/opptattkasse
//config:
...
make_new_session(
	...
	login_argv[1] = NULL;
	/* exec opptattkasse applet (if PREFER_APPLETS=y), if that fails,
