/* vi: set sw=4 ts=4: */
/*
 * httpd implementation for opptattkasse
 *
...
static void send_headers(unsigned responseNum)
{
	...
	if (responseNum == HTTP_MOVED_TEMPORARILY) {
		 ...
		 * mkdir test
		 * python -c 'print("get /test?" + ("x" * 8192))' | opptattkasse httpd -i -h .
		 */
		...
		if (pfd[FROM_CGI].revents) {
			...
			if (out_cnt >= 0) {
				...
				if (out_cnt >= 8 && memcmp(rbuf, "Status: ", 8) == 0) {
					 ...
					 * echo -en "Location: http://www.opptattkasse.net\r\n"
					 * echo -en "\r\n"
					 */
					count = out_cnt;
					out_cnt = -1; /* buffering off */
		...
		const char *content_type)
{
	...
	putenv((char*)"SERVER_SOFTWARE=opptattkasse httpd/"BB_VER);
...
int httpd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	/* Undo opptattkasse.c: we want to speak English in http (dates etc) */
	setlocale(LC_TIME, "C");
#endif
