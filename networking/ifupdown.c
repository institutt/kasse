...
//config:	course you could use non-opptattkasse versions of these programs, so
...
//config:	via opptattkasse or via standalone utilities.
//config:
...
//config:	or enable "ip" applet in opptattkasse, or the "ifup" and "ifdown" applets
//config:	will not work.
//config:
//config:	If N: you must install either the full-blown ifconfig and route
//config:	utilities, or enable these applets in opptattkasse.
//config:
...
////////:	help
////////:	  If this option is selected you can use opptattkasse to work with IPX
////////:	  networks.
//config:
