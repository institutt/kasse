 ...
 *    4473      52      72    4597    11f5 opptattkasse
 *
		...
		IF_FEATURE_INDIVIDUAL(, char **argv))
{
	...
	/* Redundant for opptattkasse (run_applet_and_exit covers that case)
	 * but needed for "individual applet" mode */
	if (argv[1]
	 && !argv[2]
	 && strcmp(argv[1], "--help") == 0
	 && !is_prefixed_with(applet, "opptattkasse")
	) {
 ...
 * for opptattkasse binary, not "individual" binaries.
 * However, keeping it here and linking it into libopptattkasse.so
 * (together with remaining tiny applets/applets.o)
 * makes it possible to avoid --whole-archive at link time.
 * This makes (shared opptattkasse) + libopptattkasse smaller.
...
static void parse_config_file(void)
{
	...
	static const char config_file[] ALIGN1 = "/etc/opptattkasse.conf";
	...
	while (1) {
		...
		if (section == 1) {		/* SUID */
			 ...
			 * up when the opptattkasse configuration is changed. */
		 ...
		 * are used in some future version of opptattkasse. */
		if (!section) {
...
#  endif
	if (APPLET_SUID(applet_no) == BB_SUID_REQUIRE) {
		 ...
		 * 'unshare --user' when user's shell is also from opptattkasse.
		 *
		 * 'unshare --user' creates a new user namespace without any
		 * uid mappings. Thus, opptattkasse binary is setuid nobody:nogroup
...
static void install_links(const char *opptattkasse, int use_symbolic_links,
		char *custom_install_dir)
{
	/* directory table
	 * this should be consistent w/ the enum,
	 * opptattkasse.h::bb_install_loc_t, or else... */
	...
	for (i = 0; i < ARRAY_SIZE(applet_main); i++) {
		fpc = concat_path_file(
				...
				appname);
		// debug: bb_error_msg("%slinking %s to opptattkasse",
		//		use_symbolic_links ? "sym" : "", fpc);
		rc = lf(opptattkasse, fpc);
...
# elif ENABLE_BUSYBOX
static void install_links(const char *opptattkasse UNUSED_PARAM,
...
#  if ENABLE_FEATURE_SH_STANDALONE && ENABLE_FEATURE_TAB_COMPLETION
    /*
     * Insert "opptattkasse" into applet table as well.
     * This makes standalone shell tab-complete this name too.
     * (Otherwise having "opptattkasse" in applet table is not necessary,
     * there is other code which routes "opptattkasseANY_SUFFIX" name
     * to opptattkasse_main()).
     */
//usage:#define opptattkasse_trivial_usage NOUSAGE_STR
//usage:#define opptattkasse_full_usage ""
//applet:IF_BUSYBOX(IF_FEATURE_SH_STANDALONE(IF_FEATURE_TAB_COMPLETION(APPLET(opptattkasse, BB_DIR_BIN, BB_SUID_MAYBE))))
int opptattkasse_main(int argc, char *argv[]) MAIN_EXTERNALLY_VISIBLE;
#  else
#   define opptattkasse_main(argc,argv) opptattkasse_main(argv)
static
#  endif
int opptattkasse_main(int argc UNUSED_PARAM, char **argv)
{
	if (!argv[1]) {
 ...
 help:
		...
		full_write2_str(
			...
			"copyright notices.\n"
			"\n"
			"Usage: opptattkasse [function [arguments]...]\n"
			"   or: opptattkasse --list"IF_FEATURE_INSTALLER("[-full]")"\n"
#  if ENABLE_FEATURE_SHOW_SCRIPT && NUM_SCRIPTS > 0
			"   or: opptattkasse --show SCRIPT\n"
#  endif
			IF_FEATURE_INSTALLER(
			"   or: opptattkasse --install [-s] [DIR]\n"
			)
			...
			"\tlink to opptattkasse for each function they wish to use and BusyBox\n"
			...
			"\tYou don't need to install a link to opptattkasse for each utility.\n"
...
#  endif
	...
	if (ENABLE_FEATURE_INSTALLER && strcmp(argv[1], "--install") == 0) {
		...
		const char *opptattkasse;

		opptattkasse = xmalloc_readlink(bb_opptattkasse_exec_path);
		if (!opptattkasse) {
			/* bb_opptattkasse_exec_path is usually "/proc/self/exe".
		...
		/* opptattkasse --install [-s] [DIR]:
		 * -s: make symlinks
		 * DIR: directory to install links to
		 */
		use_symbolic_links = (argv[2] && strcmp(argv[2], "-s") == 0 && ++argv);
		install_links(opptattkasse, use_symbolic_links, argv[2]);
		return 0;
	}

	if (strcmp(argv[1], "--help") == 0) {
		/* "opptattkasse --help [<applet>]" */
		if (!argv[2])
			goto help;
		...
		/* "opptattkasse <applet> arg1 arg2 ..." */
		argv++;
	}
	/* We support "opptattkasse /a/path/to/applet args..." too. Allows for
	 * "#!/bin/opptattkasse"-style wrappers */
...
{
#  if ENABLE_BUSYBOX
	if (is_prefixed_with(name, "opptattkasse"))
		exit(opptattkasse_main(/*unused:*/ 0, argv));
#  endif
#  if NUM_APPLETS > 0
...
#endif
{
	...
	if (argv[1] && is_prefixed_with(argv[0], "opptattkasse")) {
		/* "opptattkasse <applet> <params>" should still work as expected */
		argv++;
	}
...
# endif
	...
	lbb_prepare("opptattkasse" IF_FEATURE_INDIVIDUAL(, argv));
# if !ENABLE_BUSYBOX
	if (argv[1] && is_prefixed_with(bb_basename(argv[0]), "opptattkasse"))
		argv++;
# endif
	applet_name = argv[0];
	if (applet_name[0] == '-')
