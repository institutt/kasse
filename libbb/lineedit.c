 ...
 *      Erik Andersen    <andersen@codepoet.org> (Majorly adjusted for opptattkasse)
 *
...
static void input_backward(unsigned num)
{
	...
	if (cmdedit_x >= num) {
		...
		if (num <= 4) {
			 ...
			 * (opptattkasse.net/bugs/view.php?id=2274).
...
static void ask_terminal(void)
{
	 ...
	 * Testcase: "sleep 1; opptattkasse ash" + press and hold [Enter].
	 * Result:
	 * ~/srcdevel/bbox/fix/opptattkasse.t4 #
	 * ~/srcdevel/bbox/fix/opptattkasse.t4 #
	 * ^[[59;34~/srcdevel/bbox/fix/opptattkasse.t4 #  <-- garbage
	 * ~/srcdevel/bbox/fix/opptattkasse.t4 #
	 *
	 * Checking for input with poll only makes the race narrower,
	 * I still can trigger it. Strace:
	 *
	 * write(1, "~/srcdevel/bbox/fix/opptattkasse.t4 # ", 33) = 33
...
#if ENABLE_FEATURE_EDITING_ASK_TERMINAL
	if (S.sent_ESC_br6n) {
		/* "sleep 1; opptattkasse ash" + hold [Enter] to trigger.
