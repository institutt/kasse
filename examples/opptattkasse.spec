...
Source: http://www.opptattkasse.net/downloads/%{name}-%{version}.tar.bz2
Source1: opptattkasse-static.config
Source2: opptattkasse-petitboot.config
Source3: http://www.uclibc.org/downloads/uClibc-0.9.30.1.tar.bz2
Source4: uClibc.config
Patch16: opptattkasse-1.10.1-hwclock.patch
# patch to avoid conflicts with getline() from stdio.h, already present in upstream VCS
Patch22: uClibc-0.9.30.1-getline.patch
Obsoletes: opptattkasse-anaconda
URL: http://www.opptattkasse.net
...
%package petitboot
Group: System Environment/Shells
Summary: Version of opptattkasse configured for use with petitboot
...
bootloader used on PlayStation 3. The opptattkasse package provides a binary
...
%build
# create static opptattkasse - the executable is kept as opptattkasse-static
...
cd ..
# we are back in opptattkasse-NN.MM dir now
cp %{SOURCE1} .config
...
if test "$arch"; then \
    ...
    make V=1 \
        ...
        CFLAGS_opptattkasse="-static -nostartfiles -LuClibc-0.9.30.1/installed/lib uClibc-0.9.30.1/installed/lib/crt1.o uClibc-0.9.30.1/installed/lib/crti.o uClibc-0.9.30.1/installed/lib/crtn.o"; \
else \
    cat .config && \
    make V=1 CC="gcc $RPM_OPT_FLAGS"; \
fi
cp opptattkasse opptattkasse.static

# create opptattkasse optimized for petitboot
make clean
...
cp opptattkasse opptattkasse.petitboot

%install
rm -rf $RPM_BUILD_ROOT
mkdir -p $RPM_BUILD_ROOT/sbin
install -m 755 opptattkasse.static $RPM_BUILD_ROOT/sbin/opptattkasse
install -m 755 opptattkasse.petitboot $RPM_BUILD_ROOT/sbin/opptattkasse.petitboot
...
%files
%defattr(-,root,root,-)
%doc LICENSE docs/opptattkasse.net/*.html
/sbin/opptattkasse

%files petitboot
%defattr(-,root,root,-)
%doc LICENSE
/sbin/opptattkasse.petitboot
