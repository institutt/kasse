...
/* If using opptattkasse's shadow implementation, do not include the shadow.h
...
/* We can just memorize it once - no multithreading in opptattkasse :) */
 ...
 * but it may exec opptattkasse and call applet instead of searching PATH.
 */
...
#define BB_EXECLP(prog,cmd,...) \
	do { \
		if (find_applet_by_name(prog) >= 0) \
			execlp(bb_opptattkasse_exec_path, cmd, __VA_ARGS__); \
...
/* We need to export XXX_main from libopptattkasse
...
extern const char bb_opptattkasse_exec_path[] ALIGN1;
