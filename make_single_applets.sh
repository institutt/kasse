...
for app; do
	...
	if ! yes '' | make oldconfig >opptattkasse_make_${app}.log 2>&1; then
		fail=$((fail+1))
		echo "Config error for ${app}"
		mv .config opptattkasse_config_${app}
	elif ! make $makeopts >>opptattkasse_make_${app}.log 2>&1; then
		fail=$((fail+1))
		grep -i -e error: -e warning: opptattkasse_make_${app}.log
		echo "Build error for ${app}"
		mv .config opptattkasse_config_${app}
	elif ! grep -q '^#define NUM_APPLETS 1$' include/NUM_APPLETS.h; then
		grep -i -e error: -e warning: opptattkasse_make_${app}.log
		mv opptattkasse opptattkasse_${app}
		fail=$((fail+1))
		echo "NUM_APPLETS != 1 for ${app}: `cat include/NUM_APPLETS.h`"
		mv .config opptattkasse_config_${app}
	else
		grep -i -e error: -e warning: opptattkasse_make_${app}.log \
		|| rm opptattkasse_make_${app}.log
		mv opptattkasse opptattkasse_${app}
		#mv .config opptattkasse_config_${app}
	fi
	mv .config.SV .config
