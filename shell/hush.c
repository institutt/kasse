 ...
 *      support has been adapted from opptattkasse-0.49pre's lash, which is
...
#include "opptattkasse.h"  /* for APPLET_IS_NOFORK/NOEXEC */
#include "unicode.h"
...
# endif
 ...
 do_exec:
	...
	execve(bb_opptattkasse_exec_path, argv, pp);
...
# endif
	...
	/* Check if the command matches any opptattkasse applets */
	{
...
//if you replace "opptattkasse find" with just "find" in them
...
# endif
			...
			if (SPECIAL_JOBSTOP_SIGS != 0)
				switch_off_special_sigs(G.special_sig_mask & SPECIAL_JOBSTOP_SIGS);
			execv(bb_opptattkasse_exec_path, argv);
